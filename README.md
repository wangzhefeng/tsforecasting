...
`tsforecasting` 是基于 Nixtla 栈的统一时间序列预测框架。**MVP-0（StatsForecast 纵切面）与 MVP-1 的 MLForecast 后端已实现**：单 YAML 配置 → 校验 → CSV 转 Nixtla long table → 训练/回测 → UtilsForecast 统一评估 → 统一 artifacts + manifest。StatsForecast 与 MLForecast 模型可在同一次 run 中跨 backend 统一排名。
...
- Extras：`ml`（MLForecast + scikit-learn，**已实现**）/ `neural` / `hierarchical` / `plot`（后三者尚未实现）；dev 组：`pytest`、`ruff`。
...
示例配置用小时级 `examples/ett_small/ETTh1.csv`（`freq: 1h` / `season_length: 24`）：`configs/examples/ett_small_stats.yaml`（StatsForecast `SeasonalNaive`/`AutoETS`）、`configs/examples/ett_small_ml.yaml`（混合 StatsForecast + MLForecast，需 `uv sync --extra ml`）。`run` 默认跑回测 + 评估；配置含 `predict.horizon` 时额外产出 `predictions.csv`。运行级 override：`--run-id` / `--output-dir` / `--log-name` / `--log-level` / `--dry-run`。
...
- [docs/PLAN.md](docs/PLAN.md) — 可执行开发计划和计划项实现记录（MVP-0 P1–P6、MVP-1 P7 done）。
...
- MVP-1：MLForecast（已完成）、NeuralForecast CPU smoke、TourismSmall hierarchical reconciliation。
